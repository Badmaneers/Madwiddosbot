...
def send_welcome(message):
    bot.reply_to(message, "Welcome to the UDP Bot! Please use the command /run to start.")

@bot.message_handler(commands=['run'])
def handle_run(message):
    msg = bot.reply_to(message, "Please provide the following inputs separated by spaces:\n1. IP:Port\n2. Thread\n3. Time")
...
def process_inputs(message):
    try:
        ...
        command = f"python start.py udp {domain} {threads} {duration}"
